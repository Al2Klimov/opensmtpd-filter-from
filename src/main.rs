...
use cli::{blame_user, parse_cmdline};
use mail_parser::MessageParser;
use std::collections::HashMap;
use std::env::args_os;
use std::io::{BufRead, Result as IoResult, Write, stderr, stdin, stdout};
use std::process::exit;
use util::join_write_bytes;

fn main() -> IoResult<()> {
    let (_, rblacklist, consumed) = parse_cmdline(args_os());
    let (addrs, domains) = match rblacklist {
        Err(err) => {
                            ...
                            writeln!(
                                ...
                                if match sessions.get(session) {
                                    ...
                                    Some(mail) => match MessageParser::new().parse_headers(mail) {
                                        ...
                                        Some(mail) => match mail.from() {
                                            None => {
                                                writeln!(std_err, "No From: header")?;
                                                true
                                            }
                                            Some(from) => {
                                                let mut allow = true;

                                                for frm in from.iter() {
                                                    match &frm.address {
                                                        None => {}
                                                        Some(addr) => {
                                                            if addrs.contains(addr.as_ref()) {
                                                                writeln!(
                                                                    std_err,
                                                            ...
                                                            for domain in &domains {
                                                                if addr.ends_with(domain.as_str()) {
                                                                    writeln!(
                                                                        std_err,
                                                                        "Sender domain is blacklisted"
                                                                    )?;
